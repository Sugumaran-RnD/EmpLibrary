...
import com.nibav.commons.databinding.DialogTitleBinding
import com.nibav.commons.dialogs.*
import com.nibav.commons.dialogs.WritePermissionDialog.Mode
import com.nibav.commons.helpers.*
import com.nibav.commons.models.*
import com.nibav.commons.views.MyTextView
import java.io.*
import java.util.*
...
fun Activity.launchPurchaseThankYouIntent() {
    hideKeyboard()
    try {
        launchViewIntent("market://details?id=com.nibav.thankyou")
...
fun Activity.openPathIntent(
    ...
    ensureBackgroundThread {
        ...
        Intent().apply {
            ...
            if (applicationId == "com.nibav.gallery.pro" || applicationId == "com.nibav.gallery.pro.debug") {
...
fun BaseSimpleActivity.launchCallIntent(recipient: String, handle: PhoneAccountHandle? = null) {
    handlePermission(PERMISSION_CALL_PHONE) {
        ...
        Intent(action).apply {
            ...
            if (isDefaultDialer()) {
                val packageName = if (baseConfig.appId.contains(".debug", true)) "com.nibav.dialer.debug" else "com.nibav.dialer"
                val className = "com.nibav.dialer.activities.DialerActivity"
